...
Page({
      ...
      success: function (res) {
        if (res.state == "0") {
          ...
          WxParse.wxParse("detail", "html", a, page), page.setData({
            ...
            presellInfo: res.data.marketingInfo.presellInfo
          });
  ...
  numberAdd: function () {
    ...
    if (++a > parseInt(t.data.presellInfo.limitAmount) && 0 != parseInt(t.data.presellInfo.limitAmount)) return wx.showToast({
      title: "一单限购" + t.data.presellInfo.limitAmount + "件",
  ...
  numberBlur: function (e) {
    ...
    else if (parseInt(that.data.presellInfo.limitAmount) < num && parseInt(that.data.presellInfo.limitAmount) < num != 0) (wx.showToast({
      title: "一单限购" + that.data.presellInfo.limitAmount + "件",
      image: "/images/icon-warning.png"
    }), num = parseInt(that.data.presellInfo.limitAmount)); that.setData({
      form: {
        number: num
      }
    });
  ...
  submit: function (t) {
    var a = this;
    if (a.data.form.number > a.data.presellInfo.currentStock) return a.showToast({
      title: "商品库存不足，请选择其它规格或数量",
      image: "/images/icon-warning.png"
    }), !0;
    "BUY_NOW" == t && app.request({
      url: api.order.check_order,
      data: { "buyType": "5", "actId": a.data.presellInfo.id, "count": a.data.form.number, "pid": a.data.id},
      success: function (res) {
        if(res.state == "0") {
          wx.navigateTo({
            url: "/pages/yushou/order_submit/order_submit?goods_info=" + JSON.stringify({
              goods_id: a.data.id,
              num: a.data.form.number,
              buyType: "5",
              actId: a.data.presellInfo.id
            })
          })
        }else {
          wx.showModal({
