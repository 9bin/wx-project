// pages/limit_time_buy/detail/detail.js
...
Page({
      ...
      success: function (res) {
        if (res.state == "0") {
          ...
          WxParse.wxParse("detail", "html", a, page), page.setData({
            goods: res.data,
            flash_data: res.data.marketingInfo.flashSaleInfo
          }), page.data.flash_data && page.setFlashTimeOver();
   ...
   * 设置秒杀时间
   */
  setFlashTimeOver: function () {
    var a = this;
    function t() {
      var t = a.data.flash_data.flashSaleEndTime || 0;
      a.data.flash_data.flashSaleEndTime-- , t = t < 0 ? 0 : t, a.setData({
        flash_data: a.data.flash_data,
        flash_end_time_over: function (t) {
  ...
  numberAdd: function () {
    ...
    if (++a > t.data.flash_data.useLimit && 0 != t.data.flash_data.useLimit) return wx.showToast({
      title: "一单限购" + t.data.flash_data.useLimit + "件",
  ...
  numberBlur: function (e) {
    ...
    else if (that.data.flash_data.useLimit < num && that.data.flash_data.useLimit < num != 0) (wx.showToast({
      title: "一单限购" + that.data.flash_data.useLimit + "件",
      image: "/images/icon-warning.png"
    }), num = that.data.flash_data.useLimit); that.setData({
      form: {
        number: num
      }
    });
   ...
   */
  buyNow: function () {
    this.data.flash_data ? this.submit("BUY_NOW") : wx.showModal({
      title: "提示",
      content: "商品当前时间暂无活动",
      showCancel: !1,
    });
  ...
  submit: function (t) {
    var a = this;
    if (a.data.flash_data && 0 < a.data.flash_data.currentStock && a.data.form.number > a.data.flash_data.currentStock) return wx.showToast({
    ...
    "BUY_NOW" == t && app.request({
      ...
      data: { "buyType": "2", "actId": a.data.flash_data.id, "count": a.data.form.number, "pid": a.data.id },
      success: function (res) {
        if (res.state == "0") {
          wx.navigateTo({
            url: "/pages/limit_time_buy/order_submit/order_submit?goods_info=" + JSON.stringify({
              goods_id: a.data.id,
              num: a.data.form.number,
              buyType: "2",
              actId: a.data.flash_data.id
            })
          });
        } else {
          wx.showModal({
